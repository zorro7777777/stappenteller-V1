...
Ruwe GPS fix
  ...
  → Filter 2: Mediaan van 5 fixes          → jitter onderdrukken
  → Filter 3: Afstand < 1m of > 150m      → ruis / teleport, skip
  → Filter 4: Snelheid < 0.37 m/s         → stilstand, skip
  → Filter 5: Snelheid > 1.944 m/s        → te snel (>7km/u), skip
  → Stappen berekenen
```
...
├── logic.js            ← Stappenteller-logica v3.0  (app-specifiek)
